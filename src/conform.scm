...
(define (clean-graph g)
  (define (clean-node node)
    (unless (or (any-node? node) (none-node? node))
    ...
    (define (fix-set object selector mutator)
      (mutator object
               (map (lambda (node)
                    ...
                    (selector object))))
    (unless (or (none-node? node) (any-node? node))
...
(define (conforms? t1 t2)
  ...
  (define (delete-red-edges! from-node)
    (set-red-edges! from-node '()))
  (define (does-conform t1 t2 n)
    (define (space)
      (let loop ([i 0])
        (if (= i n) '()
            (begin (display "    ") (loop (+ i 1))))))
    (space)(display "does-conform\n")
    (cond ((or (none-node? t1) (any-node? t2)) (space)(display "1=>#t\n") #t)
          ((or (any-node? t1) (none-node? t2)) (space)(display "2=>#f\n") #f)
          ((green-edge? t1 t2) (space)(display "3=>#t\n") #t)
          ((red-edge? t1 t2) (space)(display "4=>#t\n") #t)
          (else
          (space)(display "else\n")
           (add-red-edge! t1 t2)
           (let loop ((blues (blue-edges t2)))
             (if (null? blues)
                 (begin (space)(display "=>else out\n") #t)
                 (let* ((current-edge (car blues))
                        (phi (operation current-edge)))
                   (and (has-op? phi t1)
                        (space) (display "=> recusive1\n")
                        (does-conform
                         (res (sig phi t1))
                         (res (sig phi t2)) (+ n 1))
                        (space)(display "=> recusive2\n")                         
                        (does-conform
                         (arg (sig phi t2))
                         (arg (sig phi t1)) (+ n 1))
                        (space)(display "=> loop\n")                         
                        (loop (cdr blues)))))))))
  (let ((result (does-conform t1 t2 0)))
    (newline)
    (display "final\n")
    (display result)
    (exit)
